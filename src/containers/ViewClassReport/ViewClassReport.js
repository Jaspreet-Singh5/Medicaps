...
class ViewClassReport extends Component {
    ...
    if (!this.state.loading && this.props.fees) {
      ...
      if (this.state.feesCollection.length == 0) {
        ...
        this.props.fees.forEach(fee => {
          ...
          if (fee.studentName.sectionName === this.props.match.params.cid) {
            let paidAmount = 0;
            let dueAmount = 0;
...
export default connect(mapStateToProps, mapDispatchToProps)(withErrorHandler(withStyles(styles)(ViewClassReport), axios));